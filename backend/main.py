...
async def generate(
    ...
    try:
        ...
        if payload.input_type in ['code', 'requirement'] and payload.code and len(payload.code) >= 10:
            try:
                ...
                print(f"[FALLBACK] analyze_code failed: {e}")
                code_analysis = {}
        ...
        if payload.input_type == 'url' and payload.url:
            try:
                try:
                    ...
                    print(f"[FALLBACK] analyze_url failed: {e}")
                    ui_analysis = {}
            ...
            try:
                try:
                    ...
                    print(f"[FALLBACK] analyze_screenshot failed: {e}")
                    ui_analysis = {}
                functional_tests = generate_functional_test_cases(ui_analysis or {})
                ui_bugs_list = predict_ui_bugs(ui_analysis or {})
            except Exception as e:
        ...
        try:
            ...
            print(f"[FALLBACK] generate_explanation_and_improvements failed: {e}")
            improvement_data = {'explanation': '', 'improvements': []}
    ...
    return GenerateResponse(
        ...
        bug_predictions=code_bugs + ui_bugs_list,
...
def get_history(
    ...
    for r in runs:
        ...
        items.append(HistoryItem(
            ...
            input_preview=r.input_code[:100] if r.input_code else "",
            bug_count=bug_count,
        ))
...
def get_run_detail(
    ...
    analysis   = analyze_code(run.input_code or "", run.language, run.input_type)
    coverage   = evaluate_coverage(analysis, test_cases, edge_cases)

    return RunDetailResponse(
        id=run.id, title=run.title, input_code=run.input_code or "",
